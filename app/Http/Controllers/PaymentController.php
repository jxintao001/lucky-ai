...
use App\Services\AbcpayService;
...
use App\Services\UserFoodStampService;
...
class PaymentController extends Controller
{
    ...
    {
        $except = ['payment.wechat.notify', 'payment.wechat.refund_notify', 'payment.baofu.notify', 'payment.abcpay.notify'];
    ...
    public function payByAlipay($id, Request $request)
    {
        ...
        return app('alipay')->web([
            ...
            'subject'      => '支付云熠供应链订单：' . $order->no, // 订单标题
        ]);
    }
    ...
    public function payByAbc($id, Request $request, AbcpayService $abcpayService)
    {
        // 订单信息
        $order = Order::findOrFail($id);
        $user = User::findOrFail($order->user_id);
        ...
        $order->update([
            'payment_method' => 'abcpay', // 支付方式，农行支付
            'suffix'         => time(), // 订单号
        ]);
        // 之前是直接返回，现在把返回值放到一个变量里
        $wechatOrder = $abcpayService->merchanPay([
            'out_trade_no' => $order->no . '_' . $order->suffix,
            'total_fee'    => $order->total_amount,
            //'total_fee'    => 0.01,
            'body'         => '支付云熠供应链订单：' . $order->no,
    ...
    public function wechatNotify(SplitOrderService $splitOrderService, CustomsService $customsService, UserGiftService $userGiftService)
    {
        // 校验回调参数是否正确
        ...
        if ($order->type == 'normal') {
            ...
            $this->afterPaid($order);
        } else if ($order->type == Order::TYPE_RECHARGE && $order->food_stamp_set_id) {
            $food_stamp_set = FoodStampSet::query()->where('is_blocked', 0)
                ->where('id', $order->food_stamp_set_id)
                ->first();
            if ($food_stamp_set) {
                $food_stamp_amount = $food_stamp_set->food_stamp_amount + $food_stamp_set->give_amount;
                // 充值粮票
                $description = '粮票充值-' . $food_stamp_set->title;
                (new UserFoodStampService())->getFoodStamp(UserFoodStamp::ACTION_TYPE_RECHARGE, $order->user, $food_stamp_amount, $description, $order, $order->user->master_card_no);
            }
            return app('wechat_pay')->success();
        }
        // 订单拆分
        ...
        // 导入礼品库
        //$userGiftService->orderToGift($order->id);
        return app('wechat_pay')->success();
    }
    ...
    public function abcpayNotify(Request $request, AbcpayService $abcpayService)
    {
        // 校验输入参数
        $data = $abcpayService->verify();
        $orderNo = $data['OrderNo'];
        $out_trade_no = explode('_', $orderNo);
        $no = $out_trade_no[0];
        $suffix = !empty($out_trade_no[1]) ? $out_trade_no[1] : '';
        // 找到对应的订单
        $order = Order::where('no', $no)->first();
        ...
        $order->update([
            'paid_at'         => Carbon::now(),
            'payment_method'  => 'abcpay',
            'payment_no'      => $data['iRspRef'],
            'suffix'          => $suffix,
            'pay_notify_data' => $data,
        ]);
        $this->afterPaid($order);
    ...
    public function trnxRecords(Request $request){
        if (!$request->SettleDate){
            return $this->errorBadRequest('对账单日期不能为空');
        }
        $abcpayService = new AbcpayService();
        return $abcpayService->queryTrnxRecords($request);
    }
