	...
	if ($orderInfo == null) {
		...
		if ($queryType === "0") {
			...
			if ($payTypeID === "ImmediatePay" || $payTypeID === "PreAuthPay") {
				...
				//新增分账账户信息
                $orderItems = $detail->GetArrayValue("SplitAccInfoItems");
                if (count($orderItems, COUNT_NORMAL) === 0){
                    print ("分账账户信息为空<br/>");
                } else{
                    foreach ($orderItems as $orderItem) {
                        $detail = new Json($orderItem);
                        print ("分账账户信息明细为:<br/>");
                        print ("SplitAcc      = [" . $detail->GetValue("SplitAcc") . "]<br/>");
                        print ("SplitAmount      = [" . $detail->GetValue("SplitAmount") . "]<br/>");
                    }
                }
				...
				if ($payTypeID === "DividedPay") {
					...
					if ($payTypeID === "Refund") {
						...
						if ($payTypeID === "AgentPay") {
							...
							if (payTypeID === "PreAuthed" || payTypeID === "PreAuthCancel") {
								print ("PayTypeID      = [" . $detail->GetValue("PayTypeID") . "]<br/>");
								print ("OrderNo      = [" . $detail->GetValue("OrderNo") . "]<br/>");
								print ("OrderDate      = [" . $detail->GetValue("OrderDate") . "]<br/>");
								print ("OrderTime      = [" . $detail->GetValue("OrderTime") . "]<br/>");
