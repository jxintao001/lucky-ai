...
class DownloadStatement extends TrxRequest {
    public $request = array (
        "TrxType" => IFunctionID :: TRX_TYPE_DOWNLOAD_STATEMENT,
        "StatementDate" => "",
        "StatementType" => "",
        "Zip" => ""
    );
    ...
    protected function checkRequest() {
        if (empty($this->request["StatementDate"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "对账单日期未设定！");
        if (empty($this->request["StatementType"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "对账单类型未设定！");
