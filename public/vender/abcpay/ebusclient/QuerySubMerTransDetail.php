...
class QuerySubMerTransDetail extends TrxRequest {
    public $request = array (
        "TrxType" => IFunctionID :: TRX_TYPE_QUERY_SUBMER_TRANSDETAIL,
        "SubMerchantNo" => "",
        "selType" => "",
        "Status" => "",
        "sDate" => "",
    ...
    protected function checkRequest() {
        if (empty($this->request["selType"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "交易类型未设定！");
        if (empty($this->request["Status"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "交易状态未设定！");
        ...
        if (empty($this->request["sDate"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "开始日期未设定！");
        if (empty($this->request["eDate"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "结束日期未设定！");
    }
}
?>