...
class ConfirmPOSPaymentRequest extends TrxRequest{

    public $request = array (
        "TrxType" => IFunctionID :: TRX_TYPE_EBUS_POSCONFIRM_REQ,
        "SubMerchantNo" => "",
        "OrderDate" => "",
        "OrderNo" => "",
        "SequenceNo" => "",
    ...
    protected function checkRequest() {
        ...
        if (empty($this->request["SubMerchantNo"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "请输入子商户号！");
        if (empty($this->request["OrderDate"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "请输入请求日期！");
        if (empty($this->request["OrderNo"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "请输入订单编号！");
        if (empty($this->request["SequenceNo"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "请输入序列号！");
        if (empty($this->request["Amount"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "请输入交易金额！");
        if (!DataVerifier :: isValidDate($this->request["OrderDate"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "请求日期格式不正确!");
        if (!DataVerifier :: isValidAmount($this->request["Amount"], 2))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "交易金额格式不正确!");
