...
class MerQueryFileRegister extends TrxRequest
{
    public $request = array (
        "TrxType" => IFunctionID :: TRX_TYPE_QUERYFILEREGISTER,
        "BatchDate" => "",
        "BatchNo" => ""
    );
    ...
    protected function checkRequest() {
        ...
        if (empty($this->request["BatchDate"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "请输入登记日期！");
        if (empty($this->request["BatchNo"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1100, TrxException :: TRX_EXC_MSG_1100, "请输入批次号！");
