...
class MigrateSubMerToOtherSubject extends TrxRequest {
    public $request = array (
        "TrxType" => IFunctionID :: TRX_TYPE_MIGRATE_SUBMER_TOOTHERSUBJECT,
        "FromMerchantNo" => "",
        "FromSubMerchantNo" => "",
        "CustomToSubMerchantNo" => ""
    );
    ...
    protected function checkRequest() {
        if (empty($this->request["FromMerchantNo"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "原商户号未设定！");
        if (empty($this->request["FromSubMerchantNo"]))
            throw new TrxException(TrxException :: TRX_EXC_CODE_1101, TrxException :: TRX_EXC_MSG_1101, "原子商户号未设定！");
    }
}
?>