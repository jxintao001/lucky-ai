...
class RefundRequest extends TrxRequest {
    ...
    public $splitAccTemplateInfo = array ();
	...
	protected function getRequestMessage() {
        ...
        $js = $js .',"TemplateSplitAccInfo":[';
        $count1 = count($this->splitAccTemplateInfo, COUNT_NORMAL);
        for ($i = 0; $i < $count1; $i++) {
            Json :: arrayRecursive($this->splitAccTemplateInfo[$i], "urlencode", false);
            $js = $js . json_encode($this->splitAccTemplateInfo[$i]);
            if ($i < $count1 -1) {
                $js = $js . ',';
            }
        }
